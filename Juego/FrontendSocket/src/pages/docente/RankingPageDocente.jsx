...
const RankingPageDocente = () => {
  ...
  useEffect(() => {
    ...
    socket.on("rankingActualizado", () => {
      console.log("📡 Ranking actualizado por socket (docente)");
      if (selectedTemporada) {
        axiosInstance.get(`/ranking/por-temporada/${selectedTemporada}`)
          .then(res => setRanking(res.data.ranking))
          .catch(err => console.error("Error actualizando ranking:", err));
      }
    });
