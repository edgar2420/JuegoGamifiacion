...
// 📦 Ranking acumulado desde monedas y logros
  ...
  try {
    const ranking = await Estudiante.findAll({
      attributes: [
        'id',
        'nombre',
        [fn('SUM', col('monedas.cantidad')), 'cc_coins'],
        [fn('COUNT', literal('DISTINCT logros.id')), 'total_logros'],
        [fn('MAX', col('monedas.fecha')), 'ultima_actualizacion']
      ],
      include: [
